...
-- -----------------------------------------------------
-- Table `eshop`.`user_statuses`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `eshop`.`user_statuses`
(
    `id`          INT         NOT NULL,
...
(
    ...
    CONSTRAINT `fk_users_user_statuses1`
        FOREIGN KEY (`user_status_id`)
            REFERENCES `eshop`.`user_statuses` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION,
    CONSTRAINT `fk_users_user_roles1`
        FOREIGN KEY (`user_role_id`)
            REFERENCES `eshop`.`user_roles` (`id`)
            ON DELETE NO ACTION
            ON UPDATE NO ACTION
)
    ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `eshop`.`user_settings`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `eshop`.`user_settings`
(
    `id`       INT         NOT NULL AUTO_INCREMENT,
    `users_id` INT         NOT NULL,
    `language` VARCHAR(45) NULL,
    PRIMARY KEY (`id`),
    INDEX `fk_user_settings_users1_idx` (`users_id` ASC) VISIBLE,
...
DELIMITER $$
USE `eshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eshop`.`orders_has_products_BEFORE_INSERT`
    BEFORE INSERT
    ON `orders_has_products`
    FOR EACH ROW
BEGIN
...
USE `eshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eshop`.`orders_has_products_AFTER_INSERT`
    AFTER INSERT
    ON `orders_has_products`
    FOR EACH ROW
BEGIN
...
USE `eshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eshop`.`orders_has_products_BEFORE_UPDATE`
    BEFORE UPDATE
    ON `orders_has_products`
    FOR EACH ROW
BEGIN
...
USE `eshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eshop`.`orders_has_products_AFTER_UPDATE`
    AFTER UPDATE
    ON `orders_has_products`
    FOR EACH ROW
BEGIN
...
USE `eshop`$$
CREATE DEFINER = CURRENT_USER TRIGGER `eshop`.`orders_has_products_AFTER_DELETE`
    AFTER DELETE
    ON `orders_has_products`
    FOR EACH ROW
BEGIN
