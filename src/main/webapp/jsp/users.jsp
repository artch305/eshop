        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title"><fmt:message key="editUser"/>
                        <strong>${user.login}</strong></h4>
                    <button type="button" onclick="closeChangeWindow()" class="close"
                            data-dismiss="modal">×
                    </button>
                </div>
                    ...
                    <div class="container p-3 my-3 border">
                        <form action="${pageContext.request.contextPath}/users" method="post">
                            ...
                            <div class="row" style="margin: 10px 10px">
                                <div class="col-3">
                                    <strong style="margin-right: 10px"><fmt:message
                                            key="login"/> </strong>
                                </div>
                                <div class="col-6">
                                ...
                                <c:if test="${requestScope.userIdWithError == user.id && requestScope.errorMessage == 'existLogin'}">
                                    <p class="text-danger"><fmt:message
                                            key="existLoginError"/></p>
                                </c:if>
                            </div>
                            ...
                            <select class="form-control" id="sel1" name="status">
                                <option value="${active.status}" <c:if
                                        test="${user.userStatus == active}">
                                    selected
                                </c:if>>
                                    <fmt:message key="active"/>
                                </option>
                                <option value="${banned.status}" <c:if
                                        test="${user.userStatus == banned}">
                                    selected
                                </c:if>>
                                    <fmt:message key="banned"/>
                                </option>
                            ...
                            <select class="form-control" id="sel2" name="role">
                                <option value="${administrator.role}" <c:if
                                        test="${user.userRole == administrator}">
                                    selected
                                </c:if>>
                                    <fmt:message key="administrator"/>
                                </option>
                                <option value="${customer.role}" <c:if
